...
const apiInterface = (platform, token, user_id) => {
  switch (platform) {
    case "Spotify":
      return new SpotifyRequest(token, user_id);
    case "Youtube":
      return new YoutubeRequest(token, user_id);
    default:
...
class APIRequest {
  ...
  async retrievePlaylists(endpoint) {
    ...
    if (response.ok) {
      ...
      throw new Error(
        "Please make sure all of the accounts you added contain playlists."
      );
  ...
  async initializePlaylist(body, endpoint) {
    ...
    try {
      ...
      if (response) {
        ...
        throw new Error(
          "You can not create a new playlist at this time, because the quota has been exceeded."
        );
      }
    } catch (err) {
      throw new Error(err.message);
...
class SpotifyRequest extends APIRequest {
  ...
  generateSpotifyQuery(name, artists) {
    const trackName = new CleanString(name)
      .clean()
      .removeFeatures()
      .str.split(" ")
      .join("%20");
    const artist = new CleanString(artists[0])
      .clean()
      .str.split(" ")
      .join("%20");
  ...
  async firstSearchResult(name, artists) {
    ...
    const resultOne = await super.firstSearchResult(
    ...
    const trackName = new CleanString(name)
      ...
      .str.split(" ")
      .join("%20");
    return await super.firstSearchResult(
  ...
  async retrievePlaylists(id) {
    return await super.retrievePlaylists(
      `https://api.spotify.com/v1/users/${id}/playlists?limit=50`
    );
  }

  parsePlaylists(jsonData) {
    return jsonData.items.map(
      (item) =>
        new Playlist(item.id, item.name, "Spotify", {
  ...
  async retrievePlaylistTracks(playlistId) {
    ...
    const retrievePlaylistTracksAccum = async (tracks, endpoint) => {
      ...
      if (endpoint) {
        ...
        return retrievePlaylistTracksAccum(
          tracks.concat(this.parseTracksFromPlaylist(jsonData)),
          jsonData.next
        );
      } else {
        return tracks;
    ...
    const jsonData = await super.retrievePlaylistTracks(
  ...
  async createPlaylist(name, tracks, playlistNames) {
    return await super.createPlaylist(
      name,
      tracks,
      playlistNames,
  ...
  async initializePlaylist(name, playlistNames) {
    var body = JSON.stringify({
      name: name,
      description: `Playlist created from ${playlistsString(playlistNames)}`,
    });
  ...
  async addTracksToPlaylist(playlistId, tracks) {
    if (tracks.length > MAX_TRACKS) {
      throw new Error(
        "The resulting playlist can only contain a maximum of 500 tracks. Yours contained " +
          tracks.length +
          " tracks."
      );
    }
...
class YoutubeRequest extends APIRequest {
  ...
  async firstSearchResult(name, artists) {
    ...
    const resultOne = await super.firstSearchResult(
      `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&order=relevance&q=${query}&type=video`
    );
    if (resultOne) return resultOne;
    const trackName = new CleanString(name)
      .clean()
      ...
      .join("%20");
    return await super.firstSearchResult(
      `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&order=relevance&q=${trackName}&type=video`
    );
  }
    ...
    return jsonData.items.map(
      (item) =>
        new Track(
          item.snippet.title,
          "Youtube",
          this.extractArtists(item.snippet.title, item.snippet.channelTitle),
          { resourceId: { kind: item.id.kind, videoId: item.id.videoId } }
        )
    );
  }

  async retrievePlaylists() {
    var playlists = await super.retrievePlaylists(
      `https://www.googleapis.com/youtube/v3/playlists?part=id&part=snippet&part=contentDetails&mine=true&maxResults=50`
    );
    // Add the liked playlist too
    playlists.unshift(
      new Playlist("liked-videos", "Liked Videos", "Youtube", { liked: true })
    );
    return playlists;
  }
  ...
  parsePlaylists(jsonData) {
    return jsonData.items.map(
      (item) =>
        new Playlist(item.id, item.snippet.title, "Youtube", {
          numTracks: item.contentDetails ? item.contentDetails.itemCount : null,
        })
    );
  }
  ...
  extractArtists(trackName, channelName) {
    ...
    let features = (trackName) => {
      ...
      const potentialFeatureIndices = featureStrings.map((str) =>
        trackName.indexOf(str)
      );
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (beginningIndex !== -1) {
          ...
          return trackName
            .substring(beginningIndex + featureStrings[i].length, endingIndex)
            .split(", ");
        }
      }
    ...
    if (playlistId === "liked-videos") {
      ...
      const retrieveLikedTracksAccum = async (tracks, endpoint) => {
        ...
        if (endpoint) {
          ...
          const nextEndpoint = jsonData.nextPageToken
            ? `https://www.googleapis.com/youtube/v3/videos?part=snippet&myRating=like&maxResults=50&pageToken=${jsonData.nextPageToken}`
            : null;
          return retrieveLikedTracksAccum(
            tracks.concat(this.parseTracksFromLikedVideos(jsonData)),
            nextEndpoint
          );
        } else {
          return tracks;
      ...
      // Retrieves the first tracks (up to 100)
      const jsonData = await this.retrieveLikedTracks(
      ...
      const retrievePlaylistTracksAccum = async (tracks, endpoint) => {
        ...
        if (endpoint) {
          const jsonData = await super.retrievePlaylistTracks(endpoint);
          const nextEndpoint = jsonData.nextPageToken
            ? `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${playlistId}&maxResults=50&pageToken=${jsonData.nextPageToken}`
            : null;
          return retrievePlaylistTracksAccum(
            tracks.concat(this.parseTracksFromPlaylist(jsonData)),
            nextEndpoint
    ...
    return jsonData.items.map(
      (item) =>
        new Track(
          ...
          "Youtube",
          this.extractArtists(item.snippet.title, item.snippet.channelTitle),
          { resourceId: { kind: item.kind, videoId: item.id } }
        )
    );
  }
      ...
      if (tracks.length === index) {
        ...
        var body = JSON.stringify({
          snippet: {
            ...
            resourceId: tracks[index].resourceId,
          },
        });
        var requestOptions = {
          method: "POST",
