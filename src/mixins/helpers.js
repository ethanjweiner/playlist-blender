...
const main = async (destinationAccount, destinationName) => {
  ...
  if (state.selectedPlaylists.length > 5) {
    throw new Error(
      "You can only select at most 5 playlists. You selected " +
        state.selectedPlaylists.length +
        "."
    );
  }

  const numInputTracks = state.selectedPlaylists.reduce(
    (totalTracks, playlist) =>
      playlist.numTracks ? totalTracks + playlist.numTracks : totalTracks,
    0
  );

  if (numInputTracks > MAX_INPUT_TRACKS)
    throw new Error(
      "Your selected playlists can only contain a maximum of 1500 tracks total. Please choose smaller playlists."
    );
  ...
  const destinationRequest = apiInterface(
    destinationAccount.platform,
    destinationAccount.token,
    destinationAccount.id
  );
  try {
    const { url } = await destinationRequest.createPlaylist(
      destinationName,
      tracksToPost,
      playlistsToMerge.map((playlist) => playlist.name)
    );
...
const merge = async ({
  ...
  switch (mergeOption) {
    case "common-songs":
      return await commonTracksByTitle(
        platform,
        token,
        playlistsToMerge,
        minCommon
      ); // Provide a better _minCommon_ at some point
    case "common-artists":
      return await commonTracksByArtist(
        platform,
        token,
        playlistsToMerge,
        minCommon
    ...
    case "union":
      return await union(platform, token, playlistsToMerge);
    default:
      [];
  }
};
